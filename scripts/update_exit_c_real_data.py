...
def update_exit_c_html():
    # 載入真實統計數據
    with open('season3_real_stats.json', 'r', encoding='utf-8') as f:
        stats = json.load(f)
    
    exit_c_data = stats['逃生入口C']
    player_stats = exit_c_data['player_stats']
    set_top_players = exit_c_data['set_top_players']
    ...
    # 生成新的JavaScript代碼
    js_set_data = "const exitCSetData = [\n"
    for set_num in range(1, 17):
    ...
    # 生成新的排行榜數據
    js_top_players = "const exitCSetTopPlayers = {\n"
    for set_num in range(1, 17):
    ...
    # 讀取現有HTML文件
    with open('pages/advance/shops/exit-c.html', 'r', encoding='utf-8') as f:
        html_content = f.read()
    
    # 替換SET數據
    import re
    set_data_pattern = r'const exitCSetData = \[([\s\S]*?)\];'
    html_content = re.sub(set_data_pattern, js_set_data, html_content)
    
    # 替換排行榜數據
    top_players_pattern = r'const exitCSetTopPlayers = \{([\s\S]*?)\};'
    html_content = re.sub(top_players_pattern, js_top_players, html_content)
    
    # 寫回文件
    with open('pages/advance/shops/exit-c.html', 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print("✅ 逃生入口C的HTML文件已更新為真實數據")
    
    # 顯示統計摘要
    print("\n=== 逃生入口C Season3 真實數據摘要 ===")
