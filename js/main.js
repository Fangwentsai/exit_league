...
function showMatchDetails(gameUrl) {
    ...
    fetch(gameUrl)
        ...
        .then(function (html) {
            ...
            // 建立關閉按鈕（和 match-preview 一樣在卡片頂部）
            var closeHtml = '<div style="display:flex;justify-content:flex-end;padding:8px 10px;position:sticky;top:0;z-index:10;background:rgba(255,255,255,0.95);">' +
                '<button id="matchDetailCloseBtn" style="width:32px;height:32px;font-size:18px;display:flex;justify-content:center;align-items:center;background:#f44336;color:#fff;border:none;border-radius:50%;cursor:pointer;box-shadow:0 2px 5px rgba(0,0,0,0.3);">✕</button>' +
                '</div>';

            card.innerHTML = closeHtml + body.innerHTML;
