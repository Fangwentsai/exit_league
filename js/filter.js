...
function initializeFilters() {
    console.log('手動初始化篩選功能 - filter.js中的函數');
...
// 在腳本加載完成時輸出到控制台，確認函數已經導出到全局
console.log('filter.js已加載完成，函數已導出到全局範圍');
console.log('window.initializeFilters:', typeof window.initializeFilters === 'function' ? '可用' : '未定義');
console.log('window.setupScheduleFilters:', typeof window.setupScheduleFilters === 'function' ? '可用' : '未定義');
console.log('window.filterScheduleTable:', typeof window.filterScheduleTable === 'function' ? '可用' : '未定義');

// DOM加載完成後也輸出一次檢查
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOMContentLoaded - filter.js函數檢查:');
    console.log('window.initializeFilters:', typeof window.initializeFilters === 'function' ? '可用' : '未定義');
    console.log('window.setupScheduleFilters:', typeof window.setupScheduleFilters === 'function' ? '可用' : '未定義');
    console.log('window.filterScheduleTable:', typeof window.filterScheduleTable === 'function' ? '可用' : '未定義');
    ...
    setTimeout(function() {
        if (typeof window.initializeFilters === 'function') {
            console.log('DOMContentLoaded後延遲調用setupScheduleFilters');
            setupScheduleFilters();
        }
    }, 500);
});
