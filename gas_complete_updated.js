...
function doPost(e) {
  ...
  try {
    ...
    if (data.htmlContent && data.gameId) {
      try {
        // 🔥 修改：使用日期判斷賽季
        const season = getSeasonFromGameId(data.gameId, data.gameDate) || 'season6';
        ...
        Logger.log('📅 比賽日期: ' + (data.gameDate || '未提供'));
        Logger.log('🏆 判斷賽季: ' + season);
...
/**
 * 🔥 根據比賽日期判斷賽季
 * - 2026/1/27 之前 → season5
 * - 2026/1/27 開始 → season6
 * 
 * @param {string} gameId - 比賽 ID（如 g01, g89）
 * @param {string} gameDate - 比賽日期（如 2026/1/28 或 2026-1-28）
 * @returns {string} 賽季名稱（如 season5, season6）
 */
function getSeasonFromGameId(gameId, gameDate) {
  // 如果有提供日期，用日期判斷
  if (gameDate) {
    try {
      const parts = gameDate.replace(/-/g, '/').split('/');
      const date = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
      const season6Start = new Date(2026, 0, 27); // 2026/1/27
      
      Logger.log('📅 解析日期: ' + gameDate + ' → ' + date.toISOString());
      Logger.log('📅 第六季開始: ' + season6Start.toISOString());
      
      if (date >= season6Start) {
        return 'season6';
      } else {
        return 'season5';
      }
    } catch (e) {
      Logger.log('⚠️ 日期解析錯誤: ' + e.toString());
    }
  }
  
  // 如果沒有日期或解析失敗，預設 season6
  return 'season6';
}
...
function testGitHubUpload() {
  const testContent = '<!DOCTYPE html><html><head><title>Test</title></head><body><h1>Test File - ' + new Date().toISOString() + '</h1></body></html>';
  const testPath = 'game_result/season6/test_upload.html';
 ...
 * 測試函數：測試日期判斷賽季
 */
function testSeasonFromDate() {
  const testCases = [
    { gameId: 'g50', gameDate: '2025/12/25', expected: 'season5' },
    { gameId: 'g56', gameDate: '2026/1/26', expected: 'season5' },
    { gameId: 'g01', gameDate: '2026/1/27', expected: 'season6' },
    { gameId: 'g02', gameDate: '2026/1/28', expected: 'season6' },
    { gameId: 'g03', gameDate: '2026-1-29', expected: 'season6' },
  ];
  
  Logger.log('🧪 開始測試日期判斷賽季...');
  
  testCases.forEach(tc => {
    const result = getSeasonFromGameId(tc.gameId, tc.gameDate);
    const pass = result === tc.expected ? '✅' : '❌';
    Logger.log(`${pass} ${tc.gameDate} → ${result} (期望: ${tc.expected})`);
  });
}
...
4. 試算表 ID
   ...
   - 如果需要修改，請修改第 35 行的 spreadsheetId

5. 賽季判斷邏輯
   - 2026/1/27 之前 → season5（第五季）
   - 2026/1/27 開始 → season6（第六季）
