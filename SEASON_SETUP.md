...
| 欄位 | 說明 |
|------|------|
| `startDate` | 開打日 `YYYY/M/D`。試算表日期欄只填「M/D」時，用來推算年份（見下方跨年說明）。填 `null` 代表不補年份（試算表本身已有完整日期，第三屆就是這樣） |
...
| 屆數 | 實際期間 | 是否跨年 |
|------|---------|---------|
| 第三屆 | 2024/11 – 2025/2 | ✅ 跨年 |
| 第四屆 | 2025/4 – 2025/6 | 否 |
| 第五屆 | 2025/8 – 2025/12 | 否 |
| 第六屆 | 2026/1 – 2026/7 | 否 |
| **第七屆** | **2026/9 – 2027 初** | ✅ **跨年** |

`resolveSeasonYear()` 的規則是：**月份 ≥ 開打月份 → 開打年；否則 → 開打年 + 1**。
...
> ⚠️ 年份錯誤不只是顯示問題，還會影響**賽程表的過去／未來判定**，以及 news 頁「上週戰況／近期比賽」抓哪一場。設定新賽季時務必確認 `startDate` 的月份正確。
...
- [ ] **跨年賽季**：確認賽季後段（隔年 1、2 月）的日期年份正確，不是還停在開打年
